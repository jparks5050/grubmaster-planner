...
export default function App() {
  const [phase, setPhase] = useState("boot"); // boot | waiting-auth | signed-out | signed-in | error
  const [userEmail, setUserEmail] = useState("");
  const [err, setErr] = useState(null);

  // Show env status on screen to catch missing values
  ...
  useEffect(() => {
    try {
      console.log("[App] Starting. Env:", envStatus, cfg);
  ...
  const Wrap = ({ children }) => (
    ...
    >
      {children}
      <div style={{ marginTop: 24, color: "#aaa", fontSize: 12 }}>
  ...
  if (phase === "boot" || phase === "waiting-auth") {
    return (
      <Wrap>
        <h1>⏳ Loading Firebase auth…</h1>
        <p>Waiting for onAuthStateChanged callback.</p>
      </Wrap>
    );
  }
  ...
  if (phase === "error") {
    return (
      <Wrap>
        <h1 style={{ color: "salmon" }}>⚠️ Auth error</h1>
        <pre style={{ whiteSpace: "pre-wrap" }}>
          {String(err?.message || err)}
        </pre>
        <p>
          Double-check Vercel env vars: <code>VITE_FIREBASE_*</code> must be
          present and match your Firebase project.
        </p>
  ...
  return (
    <Wrap>
      ...
      {/* Place your real app UI below this line */}
      <div style={{ marginTop: 24 }}>
        <h2>App content goes here</h2>
        <p>Now that auth works, you can render the planner.</p>
      </div>
    </Wrap>
  );
}
